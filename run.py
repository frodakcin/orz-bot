...
class MyClient(discord.Client):
    async def on_ready(self):
        ...
        messages = json.loads(requests.get(
            f'https://discordapp.com/api/v6/channels/{potdStatusChannelID}/messages', headers={'authorization': token}).text)
        for i in messages:
            var = i['content']
    ...
    async def on_message(self, message):
        ...
        if censor.enabled:
            if await censor.isCensored(content.lower()):
                print(
                    f'Message \'{content}\' from user {message.author.name} has been deleted.')
